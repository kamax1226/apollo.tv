            ...
            <b-col sm="12" class="overflow-hidden">
                <div class="trending-info align-items-center w-100 animated fadeInUp">
                    <h1 class="trending-text big-title">
                        Select Year for TV Shows
                    </h1>
                    <div class="iq-custom-select d-inline-block sea-epi">
                        <V-select v-model="selected" @input="setSelected" :options="yearOption" />
                    </div>
                </div>
                ...
                <div class="favorites-contents">
                    ...
                    <VueSlickCarousel v-else class="favorites-slider list-inline  row p-0 mb-0" ref="dSlick" v-bind="sliderOption">
...
export default {
    ...
    async fetch() {
        let subval = this.selected;
        if (subval == 2021 || subval == null) subval = 0;
        let data = {
            year: 2021 - subval,
            limit: this.limit,
            offset: this.offset
        };
        this.items = await this.$store.dispatch("loadYearTvShowList", data);
    },
    data() {
        return {
            items: [],
    ...
    methods: {
        ...
        async fetch_data (offset, limit) {
            ...
            this.newItems = await this.$store.dispatch("loadYearTvShowList", data);
