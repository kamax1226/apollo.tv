    ...
    <div class="favorites-contents">
        ...
        <VueSlickCarousel v-else class="favorites-slider list-inline row p-0 mb-0" ref="dSlick" v-bind="sliderOption" @afterChange="handleAfterChange">
            <li v-for="(item,index) in sliderData" :key="index" class="slide-item">
                <div>
                    <!-- <img v-if="showFlag" :src="require('../../../assets/images/loader1.gif')" class="processing-gif"> -->
                    <!-- <img v-else :src="require('../../../assets/images/loader1.gif')"> -->
                    <div v-if="showFlag">
                        <div class="loadingio-spinner-dual-ring-v9pna4viv1d">
                            <div class="ldio-1b03m6p5bfr">
                                <div></div>
...
export default {
    ...
    async fetch() {
        ...
        this.sliderData = await this.$store.dispatch("loadCategoryChannelList", data);
    },
    data() {
        return {
            categoryData: this.category ? { ...this.category } : [],
            sliderData: [],
            sliderOption: {
                dots: false,
                arrows: false,
    ...
    methods: {
        ...
        add_favorite(id) {
            this.$confirm({
                ...
                callback: confirm => {
                    if (confirm) {
                        this.$store.dispatch("channelAddFavorite", { id: id })
                            .then(res => {
                                if (res) {
                                    this.showFlag = false;
                                    this.$notify({ group: 'all', type: 'success', title: 'Success', text: 'Adding favorite success' });
                                    this.$fetch();
                                } else {
                                    this.showFlag = false;
                                    this.$notify({ group: 'all', type: 'warn', title: 'Success', text: 'Adding favorite failed' });
                                }
                            });
                    }
                }
        ...
        remove_favorite(id) {
            this.$confirm({
                ...
                callback: confirm => {
                    if (confirm) {
                        this.$store.dispatch("channelRemoveFavorite", { id: id })
                            .then(res => {
                                if (res) {
                                    this.$notify({ group: 'all', type: 'success', title: 'Success', text: 'Removing favorite success' });
                                    this.$fetch();
                                } else {
                                    this.$notify({ group: 'all', type: 'warn', title: 'Success', text: 'Removing favorite failed' });
                                }
                            });
                    }
                }
        ...
        async fetch_data (offset, limit) {
            ...
            this.items = await this.$store.dispatch("loadCategoryChannelList", data);
